...
int getHostFromRequest(char *data,char **shost)
{
    ...
    if (p==NULL) return -1;
    p+=6;
    //printf("Value of host in request fetch is %s\n",p);
    ...
    *shost = (char *)malloc(l+4);
    ...
    //for(j=0;j<l;j++)
        //printf("byte[%d] is %x\n",j,(*shost)[j]);
    //printf("Host in getfromHost is %s\n",*shost);  
    return l;
}
...
printf("Creating if Modified Request in createifModifiedRequest \n");
    ...
    //memcpy(ifrequest,irequest,reqlen-2);
...
{   printf("Inside get_data_time\n");
	...
	else if((p = strstr(data,"Last-Modified:"))!=NULL)
	{
	    ...
	    p+=15;
	    //printf("When last modified time p is \n%c\n",*p);
		pend = strstr(p,"\r\n");
		//printf("When last modified time pend is \n%c\n",*pend);
		if (pend==NULL) return -1;
		int l = pend-p;
		//printf("len is %d\n",l);
		//char *len=NULL;
		//time = (char *)malloc(l+2);
		bzero(time,l+2);
		memcpy(time,p,l);
		//printf("after memcpy time is %s\n",time);
	...
	else
	{
		...
		bzero(time,l);
		strncpy(time,p,l);
		return false; //if time is expiretime
	}
...
bool get_data_time(char *data,char **time)
{
    ...
    if(p = strstr(data,"Expires:"))
    {
        printf("Response contains Expires time\n");
		p+=8;
		pend = strstr(p,"\r\n");
		if (pend==NULL) return -1;
		int l = pend-p;
	...
	else if(p = strstr(data,"Last-Modified:"))
	{
	            printf("Response contains LastModified time\n");
	    p+=15;
	    printf("When last modified time p is \n%c\n",*p);
		pend = strstr(p,"\r\n");
		printf("When last modified time pend is \n%c\n",*pend);
	...
	{
	    printf("Response contains Date time\n");
	    p = strstr(data,"Date:");
		p+=5;
		pend = strstr(p,"\r\n");
		if (pend==NULL) return -1;
...
int get_server_header(char *data,
                      char **header_data,int *header_len,
                      char **other_data,int *other_len)
{

    //printf("Inside get_server_header \n");
    //printf("Inside get_server_header\n");
    char *p = NULL;
    ...
    *other_data = (char *)malloc(*other_len+1);
    ...
    //free(data);
//    printf("returning successfully from get_server_header\n");
    return *header_len;
}
